...
//			gsonBuilder.setLongSerializationPolicy( LongSerializationPolicy.STRING );
	 ...
	 * @return Json字符串
	 * */
	public static String objectToMyJson(Object srcObj){
		String strJson = "";
		try {
			if (srcObj == null || TextUtils.isEmpty(srcObj.toString()))
				throw new Exception("传进来的参数不能为空！");
...
//			gsonBuilder.setLongSerializationPolicy( LongSerializationPolicy.STRING );
	 ...
	 *            实体对象
	 * */
	public static <T> T jsonToMyObject(String json, Class<T> clazz) {
		try {
			if (json == null || json.equals(""))
				return null;
			// 定义转换器
...
//			gsonBuilder.setLongSerializationPolicy( LongSerializationPolicy.STRING );
	 ...
	 *            Json格式字符串
	 * */
	public static <T> T jsonToMyObject(String json, TypeToken<T> token){
		try {
			if (json == null || json.equals(""))
				return null;
			// 定义转换器
