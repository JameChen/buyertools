package com.nahuo.live.demo.liveroom;
...
import com.nahuo.live.demo.roomutil.commondef.LoginInfo;
import com.nahuo.live.demo.roomutil.commondef.PusherInfo;
import com.nahuo.live.demo.roomutil.commondef.RoomInfo;
...
public class LiveRoom extends BaseRoom {
    ...
    public  void setRoomList(LiveDetailBean bean) {
        final ArrayList<RoomInfo> arrayList = new ArrayList<>();
        RoomInfo roomInfo=new RoomInfo();
        roomInfo.roomID=bean.getRoomID();
        roomInfo.audienceCount=bean.getWatchCount();
        roomInfo.mixedPlayURL=bean.getLiveUrl();
        roomInfo.custom="{\"praise\":1}";
        roomInfo.roomCreator=bean.getAnchorUserID()+"";
        roomInfo.roomInfo="{\"title\":\"\",\"frontcover\":\""+ ""+"\","+"\"location\":\"\"}";
        List<PusherInfo> pushers=new ArrayList<>();
        PusherInfo pusherInfo=new PusherInfo();
        pusherInfo.userID=bean.getAnchorUserID()+"";
        pusherInfo.userName=bean.getAnchorUserName();
        pusherInfo.userAvatar= Const.getShopLogo(bean.getAnchorUserID());
        pusherInfo.accelerateURL=bean.getAccelerateURL();
        pushers.add(pusherInfo);
        roomInfo.pushers=pushers;
        arrayList.add(roomInfo);
        if (mRoomList==null)
            mRoomList=new ArrayList<>();
        mRoomList.clear();
        mRoomList.addAll(arrayList);
    ...
    public void createRoom(final String roomID, final String roomInfo, final CreateRoomCallback cb) {
        ...
        mHttpRequest.getPushUrl(SpManager.getIdentifier(BWApplication.getInstance()), roomID, new HttpRequests.OnResponseCallback<HttpResponse.PushUrl>() {
            @Override
            ...
            public void onSuccess() {
                runOnUiThread(new Runnable() {
                    ...
                    public void run() {
                     ...
                     //   String mixedPlayUrl ="rtmp://47474.livepush.myqcloud.com/live/47474_im01?bizid=47474&txSecret=ca2f747e44628c7f24d2b89cad10e46f&txTime=5CC2B52D";
                        if (mixedPlayUrl != null && mixedPlayUrl.length() > 0) {
                            ...
                            if (mHttpRequest != null) {
                                ...
                                mHttpRequest.addAudience(roomID, SpManager.getIdentifier(BWApplication.getInstance()), userInfo, null);
                            }
                            callback.onSuccess();
                        }
                        else {
     ...
     * @param callback 离开房间完成的回调
     */
    public void exitDissolutionRoom(final ExitRoomCallback callback) {
        final MainCallback cb = new MainCallback<ExitRoomCallback, Object>(callback);
        ...
        //6. 退出房间：请求CGI:delete_pusher，把自己从房间成员列表里删除
        mHttpRequest.delPusher(mCurrRoomID, SpManager.getIdentifier(BWApplication.getInstance()), new HttpRequests.OnResponseCallback<HttpResponse>() {
            @Override
            public void onResponse(int retcode, @Nullable String retmsg, @Nullable HttpResponse data) {
                if (retcode == HttpResponse.CODE_OK) {
                    mRoomListenerCallback.printLog("[LiveRoom] 解散群成功");
...
//        mHttpRequest.destroyRoom(mCurrRoomID, mSelfAccountInfo.userID, new HttpRequests.OnResponseCallback<HttpResponse>() {
//            @Override
//            public void onResponse(int retcode, @Nullable String retmsg, @Nullable HttpResponse data) {
//
//            }
...
//                    mRoomListenerCallback.onGetPusherList(newPushers);
    ...
    public void rejectPKRequest(String userID, String reason) {
        ...
        try {
            ...
            if (userID == mPKPusherInfo.userID) {
                mPKPusherInfo = null;
            }
        }
        catch (Exception e) {
