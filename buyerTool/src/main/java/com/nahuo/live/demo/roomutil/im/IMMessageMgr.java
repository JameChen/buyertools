...
public class IMMessageMgr implements TIMMessageListener {
            ...
            private void reLogin() {
                login(new Callback() {
                    ...
                    public void onError(int code, String errInfo) {
                        ...
                        if (code== TCConstants.CODE_TX_KILL_OUT){
                            reLogin();
                        }else {
                            mLoginSuccess = false;
    ...
    private void login(final Callback cb){
        ...
        timUserConfig.setUserStatusListener(new TIMUserStatusListener() {
            ...
            public void onForceOffline() {
                Log.w("yu","onForceOffline");
                Intent intent=new Intent();
                intent.setAction(TCConstants.EXIT_APP);
                LocalBroadcastManager.getInstance(BWApplication.getInstance()).sendBroadcast(intent);

            }

            @Override
            public void onUserSigExpired() {
                Log.w("yu","onUserSigExpired");
                Intent intent=new Intent();
                intent.setAction(TCConstants.EXIT_APP);
                LocalBroadcastManager.getInstance(BWApplication.getInstance()).sendBroadcast(intent);
            }
    ...
    private void printDebugLog(String format, Object...args){
        ...
        try {
            ...
            if (com.nahuo.buyer.tool.BuildConfig.DEBUG)
            Log.e(TAG, log);
            if (mMessageListener != null) {
