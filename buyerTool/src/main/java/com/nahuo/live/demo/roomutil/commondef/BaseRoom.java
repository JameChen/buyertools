package com.nahuo.live.demo.roomutil.commondef;
...
import com.nahuo.live.demo.roomutil.http.HttpRequests;
import com.nahuo.live.demo.roomutil.http.HttpResponse;
import com.nahuo.live.demo.roomutil.im.IMMessageMgr;
...
public abstract class BaseRoom implements IMMessageMgr.IMMessageListener {
    ...
    public void  getSaveData(Bundle outState){
        //mHttpRequest=getDefautHttpRequests(mHttpRequest);
        mAppID = TCConstants.sdkAppID;
        if (mSelfAccountInfo==null){
            mSelfAccountInfo = new SelfAccountInfo(SpManager.getIdentifier(BWApplication.getInstance()),
                    SpManager.getUserName(BWApplication.getInstance()),
                    SpManager.getShopLogo(BWApplication.getInstance()),
                    SpManager.getUserSig(BWApplication.getInstance()),
                    TCConstants.accountType, TCConstants.sdkAppID);
        }
        if (outState!=null){
            IMMessageMgr.mConnectSuccess=  outState.getBoolean("ConnectSuccess");
            IMMessageMgr.mLoginSuccess=  outState.getBoolean("LoginSuccess");
        }
        if (!IMMessageMgr.mLoginSuccess||!IMMessageMgr.mConnectSuccess||mHttpRequest==null){
            LoginInfo loginInfo = new LoginInfo();
            loginInfo.sdkAppID = TCConstants.sdkAppID;
            loginInfo.accType = TCConstants.accountType;
            loginInfo.userID = SpManager.getIdentifier(BWApplication.getInstance());
            loginInfo.userSig = SpManager.getUserSig(BWApplication.getInstance());
            loginInfo.userName =SpManager.getUserName(BWApplication.getInstance());
            loginInfo.userAvatar = SpManager.getShopLogo(BWApplication.getInstance());
            login(TCConstants.ROOM_SVR_URL, loginInfo, new IMMessageMgr.Callback() {
                @Override
    ...
    public void setSaveData(Bundle outState){
//        outState.putBoolean("ConnectSuccess",IMMessageMgr.mConnectSuccess);
//        outState.putBoolean("LoginSuccess",IMMessageMgr.mLoginSuccess);
        if (BuildConfig.DEBUG)
        Log.e("yu","connect="+IMMessageMgr.mConnectSuccess+"---loginsucess="+IMMessageMgr.mLoginSuccess);
    }

    public HttpRequests getDefautHttpRequests(HttpRequests mHttpRequest) {
        if (mHttpRequest == null) {
            mHttpRequest = new HttpRequests(TCConstants.ROOM_SVR_URL);
            mHttpRequest.setUserID(SpManager.getIdentifier(BWApplication.getInstance()));
            mHttpRequest.setToken(SpManager.getTxToken(BWApplication.getInstance()));
        }
        return mHttpRequest;
    }
        ...
        mHttpRequest.login(loginInfo.sdkAppID, loginInfo.accType, loginInfo.userID, loginInfo.userSig, new HttpRequests.OnResponseCallback<HttpResponse.LoginResponse>() {
            ...
            public void onResponse(int retcode, @Nullable String retmsg, @Nullable HttpResponse.LoginResponse data) {
                if (retcode == 0 && data != null) {
                    SpManager.setTxToken(BWApplication.getInstance(), data.token);
                    ...
                    if (mIMMessageMgr != null) {
                        mIMMessageMgr.initialize(SpManager.getIdentifier(BWApplication.getInstance()), SpManager.getUserSig(BWApplication.getInstance()), (int) TCConstants.sdkAppID, callback);
                    }
                } else {
    ...
    public void setWaterMark(final @Nullable Bitmap bitmap) {
        if (mTXLivePusher != null) {
            TXLivePushConfig config = mTXLivePusher.getConfig();
            config.setWatermark(bitmap, 0.62f, 0.02f, 0.2f);
            mTXLivePusher.setConfig(config);
        }
    }
    ...
    protected void doCreateRoom(final String roomID, String roomInfo, final CreateRoomCallback callback) {
        mHttpRequest.createRoom(roomID, SpManager.getIdentifier(BWApplication.getInstance()), roomInfo,
                new HttpRequests.OnResponseCallback<HttpResponse.CreateRoom>() {
                    ...
                    public void onResponse(int retcode, @Nullable String retmsg, @Nullable HttpResponse.CreateRoom data) {
                        if (retcode != HttpResponse.CODE_OK || data == null || data.roomID == null) {
                            if (data != null) {
                                if (data.code == 11 && data.message.equals("房间已经存在")) {
