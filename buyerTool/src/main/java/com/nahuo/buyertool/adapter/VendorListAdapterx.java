...
public class VendorListAdapterx extends BaseAdapter {

    public Context mContext;
    public List<VendorsBean.DatasBean> mList;
    ...
    public VendorListAdapterx(Context context) {
        mContext = context;
    }

    public void setData(List<VendorsBean.DatasBean> data) {
        mList = data;
    }

    public void addDataToTail(List<VendorsBean.DatasBean> data) {
        mList.addAll(data);
    }
    ...
    // 构造函数
    public VendorListAdapterx(Context Context, List<VendorsBean.DatasBean> dataList, PullToRefreshListView pullRefreshListView) {
        mContext = Context;
    ...
    public View getView(final int arg0, View arg1, ViewGroup arg2) {
        ...
        if (mList.size() > 0) {
            ...
            if (view == null) {
                view = LayoutInflater.from(mContext).inflate(R.layout.layout_vendorx_item, arg2, false);
                holder = new ViewHolder();
                holder.shopCover = (ImageView) view.findViewById(R.id.vendor_item_shopCover);
                holder.vendor_apppy = (TextView) view.findViewById(R.id.vendor_apppy);
...
//                });
            ...
            String imageurl = "";
            imageurl = ImageUrlExtends.getImageUrl(shopCoverUrl, Const.LIST_COVER_SIZE);
            if (TextUtils.isEmpty(imageurl)) {
                holder.shopCover.setImageResource(R.drawable.empty_photo);
            } else {
            ...
            holder.vendor_apppy.setOnClickListener(new OnClickListener() {
                @Override
                public void onClick(View v) {
                    if (mListener != null) {
                        mListener.onUnJoinClick(v, arg0);
                    }
                }
            });
        }
...
//        VendorsBean.DatasBean vendorModel = mList.get(clickHolder.position);
//
