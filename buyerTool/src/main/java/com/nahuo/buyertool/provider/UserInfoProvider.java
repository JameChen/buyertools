package com.nahuo.buyertool.provider;
...
public class UserInfoProvider {
    ...
    public static void exitApp(Context context){
        try {
            //停止下载任务
            UploadManager.getInstance(context).stopAllTask();
        ...
        BWApplication.getInstance().clearActivity();
    }
    /**
...
//        return StringUtils.getValue(states, userId);
    ...
    public static void setIdentityAuthState(Context context, int userId, String state){
     // "'uid':state,'uid':state
        ...
        SpManager.setString(context, getSpKey(context, PREF_IDENTITY_AUTH_STATE), state);
//        if(state.equals(UpYunConst.IDAuthState.AUTH_PASSED)){
