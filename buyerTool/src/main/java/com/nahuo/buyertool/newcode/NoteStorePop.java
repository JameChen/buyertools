 ...
 * Time : 2016/7/8 10:34
 * Description :入仓单页面用来关联采购单的popwindow
 * ===============BuyerTool====================
 */
public class NoteStorePop extends BasePopupWindow {
    @Bind(R.id.tv_select)
    TextView mTvSelect;
    ...
    public NoteStorePop(Context context) {
        super(context);
        updateData(null);
        updateNote(null);
        setFocusable(false);
    ...
    public void updateData(List<ShopItemListModel> models) {
        ...
        for (int i = 0; i < models.size(); i++) {
            ...
            num += model.getInvInCount();//计算总件数
            purchasePrice += model.getInvInCount() * model.getOriPrice();//计算总进价
            TotalPrice += model.getInvInCount() * model.getPrice();//计算总售价
        }
    ...
    private void postDate() {
        String guanlianUrl = HttpUtils.SERVERURL + "buyertool/buyer/BatchBindPurchaseFromStore";
    ...
    private String beanToJson() {
        ...
        for (ShopItemListModel model : mActivity.getList())
            beanList.add(new PurAndOrdBean(mActivity.getSelectPurchase().getID(), model.getBillingID()));
