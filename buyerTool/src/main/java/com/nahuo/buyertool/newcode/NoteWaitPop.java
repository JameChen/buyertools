 ...
 * ===============BuyerTool====================
 */
public class NoteWaitPop extends BasePopupWindow {
    @Bind(R.id.tv_select)
    TextView mTvSelect;
    ...
    public NoteWaitPop(Context context) {
        super(context);
        updateData(null);
        updateNote(null);
        setFocusable(false);
    ...
    public void updateData(List<ShopItemListModel> models) {
        ...
        for (int i = 0; i < models.size(); i++) {
            ...
            num += model.getPayQty();//计算总件数
            purchasePrice += model.getPayQty() * model.getOriPrice();//计算总进价
            TotalPrice += model.getPayQty() * model.getPrice();//计算总售价
        }
    ...
    protected int getViewId() {
        return R.layout.pop_bookdis_wait;
    }
