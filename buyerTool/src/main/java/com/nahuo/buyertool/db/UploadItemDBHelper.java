...
import com.nahuo.buyertool.model.ItemShopInfo;
...
public class UploadItemDBHelper {
    ...
    private List<ShopItemModel> getUploadItemsByUploadCounter(Context context, int uploadCounter) {
        ...
        try {
            ...
            while (cursor.moveToNext()) {
                ...
                item.setItemTagIDS(DBHelper.getString(cursor, "tags_ids"));
                ...
                item.setIsWaitOrder(DBHelper.getBoolean(cursor, "is_wait_order"));
                item.setWaitDays(DBHelper.getInt(cursor,"wait_day"));
                items.add(item);
            }
            cursor.close();
        } catch (Exception ex) {
     ...
     * @author ZZB
     */
    public ShopItemListModelX getUploadItemByCreateTime(Context context, String createTime) {
        Cursor cursor = null;
        try {
            cursor = dbManager.Select("upload_tasklist", null, "create_time == ?", new String[] {createTime}, null,
                    null, "create_time DESC");
            while (cursor.moveToNext()) {
                ShopItemListModelX item = cursorToShopItemListModelX(cursor);
                // itmelist.add(item);
                return item;
            }
            cursor.close();
     ...
     * @author ZZB
     */
    public ShopItemListModelX getUploadItemByItemId(int itemId) {
        Cursor cursor = null;
        try {
    ...
    private ShopItemListModelX cursorToShopItemListModelX(Cursor cursor) {
        ShopItemListModelX item = new ShopItemListModelX();
     ...
     * */
    @Deprecated
    public List<ShopItemListModelX> GetUploadItems(Context context) {
        List<ShopItemListModelX> itmelist = new ArrayList<ShopItemListModelX>();
        Cursor cursor = null;
        try {
    ...
    private ContentValues GetContentValues(Context context, ShopItemModel cim) {
        ...
        cv.put("is_wait_order",cim.isWaitOrder());
        cv.put("wait_day",cim.getWaitDays());
        cv.put("tags_ids",cim.getItemTagIDS());
        Gson gson = new Gson();
