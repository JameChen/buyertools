...
public class DBHelper extends SQLiteOpenHelper {
	 ...
	 * @author PJ
	 */
	private void version23(SQLiteDatabase db){
		try {
			db.beginTransaction();
			db.execSQL("alter table upload_tasklist add column wait_day integer;");
			db.execSQL("alter table upload_tasklist add column is_wait_order integer;");
			db.setTransactionSuccessful();
		} catch (Exception e) {
			e.printStackTrace();
		} finally {
	 ...
	 * @author ALAN(数据库添加标签)
	 */
	private void version24(SQLiteDatabase db){
		try {
			db.beginTransaction();
			db.execSQL("alter table upload_tasklist add column tags_ids varchar(200);");
			db.setTransactionSuccessful();
		} catch (Exception e) {
			e.printStackTrace();
		} finally {
